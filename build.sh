...
build_docker() {
    ...
    log "[*] Building Docker image vaultdb/vaultdb:1.0.0..."
    docker build -t vaultdb/vaultdb:1.0.0 -t vaultdb/vaultdb:latest .

    IMAGE_SIZE="$(docker image inspect vaultdb/vaultdb:1.0.0 --format='{{.Size}}' 2>/dev/null || echo '?')"
    log "[OK] Docker image built."
    log "     Tag:  vaultdb/vaultdb:1.0.0"
    log "     Size: ${IMAGE_SIZE} bytes"
    log ""
    log "     Quick start:"
    log "     docker run -p 5432:5432 -p 8080:8080 vaultdb/vaultdb:1.0.0"
    log ""
