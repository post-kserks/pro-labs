...
func init() {
	types.RegisterCommand("TRUNCATE", func(stmt parser.Statement) types.Command {
		return &TruncateCommand{stmt: stmt.(*parser.TruncateStatement)}
	})
}

func (c *TruncateCommand) Execute(ctx *types.ExecutionContext) (*types.Result, error) {
...
func init() {
	types.RegisterCommand("MERGE", func(stmt parser.Statement) types.Command {
		return &MergeCommand{stmt: stmt.(*parser.MergeStatement)}
	})
}
...
func buildMergeKeyFromCombined(row storage.Row, colName string, combinedColIdx map[string]int, _ int) string {
	...
	return types.ValueToString(row[ci])
}

func (c *MergeCommand) Execute(ctx *types.ExecutionContext) (*types.Result, error) {
	dbName, err := types.RequireCurrentDB(ctx)
	if err != nil {
		return nil, err
	}
	...
	err = types.MutateUnderTableLock(ctx, dbName, c.stmt.TargetTable, func() error {
		var e error
...
func (c *MergeCommand) executeMerge(ctx *types.ExecutionContext, dbName string) (*types.Result, error) {
	// Read target table
	...
	targetRows, err = types.ApplyTxOverlay(ctx, dbName, c.stmt.TargetTable, targetRows)
	if err != nil {
		return nil, err
	}
	...
	if c.stmt.SourceValues != nil {
		...
		sourceRows, err = types.ApplyTxOverlay(ctx, dbName, c.stmt.SourceTable, sourceRows)
		if err != nil {
			return nil, err
		}
...
func (c *MergeCommand) executeSourceSubquery(ctx *types.ExecutionContext) (*types.Result, error) {
	selStmt, ok := c.stmt.SourceQuery.(*parser.SelectStatement)
	if !ok {
		return nil, fmt.Errorf("MERGE USING: unsupported source query type")
	}
	cmd, err := ctx.CreateCommand(selStmt)
	if err != nil {
		return nil, fmt.Errorf("MERGE USING: %w", err)
	}
	return cmd.Execute(ctx)
}

// buildSourceFromValues converts VALUES expressions into in-memory rows and a synthetic schema.
func (c *MergeCommand) buildSourceFromValues(ctx *types.ExecutionContext) (*storage.TableSchema, []storage.Row, error) {
	...
	for _, valueRow := range c.stmt.SourceValues {
		...
		for i, expr := range valueRow {
			val, err := types.EvalOperand(expr, emptyCombined, nil, ctx)
			if err != nil {
...
func (c *MergeCommand) executeMergeHashJoin(ctx *types.ExecutionContext, dbName string, targetRows, sourceRows []storage.Row, targetSchema, sourceSchema *storage.TableSchema, eqCols [][2]string, combinedSchema *storage.TableSchema) (int, []storage.Row, []storage.Row, error) {
	affected := 0
	var returningRows []storage.Row
	var oldRows []storage.Row
	...
	for _, sourceRow := range sourceRows {
		...
		for _, targetIdx := range targetIndices {
			...
			ok, err := types.EvalExpr(c.stmt.OnCondition, combinedRow, combinedSchema, ctx)
			if err != nil || !ok {
				continue
			}
			...
			if c.stmt.WhenMatched != nil && c.stmt.WhenMatched.Action == "UPDATE" {
				...
				for _, assign := range c.stmt.WhenMatched.Assignments {
					val, err := types.EvalOperand(assign.Value, combinedRow, combinedSchema, ctx)
					if err != nil {
						return 0, nil, nil, err
					}
...
func (c *MergeCommand) executeMergeNestedLoop(ctx *types.ExecutionContext, dbName string, targetRows, sourceRows []storage.Row, targetSchema, sourceSchema *storage.TableSchema, combinedSchema *storage.TableSchema, sourceName string) (int, []storage.Row, []storage.Row, error) {
	affected := 0
	var returningRows []storage.Row
	var oldRows []storage.Row
...
func (c *MergeCommand) executeMergeInsert(ctx *types.ExecutionContext, dbName string, sourceRow storage.Row, targetSchema, sourceSchema *storage.TableSchema, combinedSchema *storage.TableSchema) error {
	...
	for i, val := range c.stmt.WhenNotMatched.Values[0] {
		v, err := types.EvalOperand(val, combinedRowForInsert, combinedSchema, ctx)
		if err != nil {
			return err
		}
		insertValues[i] = v
	...
	if len(c.stmt.WhenNotMatched.Columns) == 0 {
		...
		for i, v := range insertValues {
			converted, err := types.NormalizeForColumn(v, targetSchema.Columns[i])
			if err != nil {
		...
		for i, colName := range c.stmt.WhenNotMatched.Columns {
			...
			if i < len(insertValues) {
				converted, err := types.NormalizeForColumn(insertValues[i], targetSchema.Columns[idx])
				if err != nil {
...
func (c *MergeCommand) executeMergeInsertSelect(ctx *types.ExecutionContext, dbName string, sourceRow storage.Row, targetSchema, sourceSchema *storage.TableSchema, combinedSchema *storage.TableSchema) error {
	...
	for i, col := range selStmt.Columns {
		v, err := types.EvalOperand(col.Expr, combinedRowForInsert, combinedSchema, ctx)
		if err != nil {
