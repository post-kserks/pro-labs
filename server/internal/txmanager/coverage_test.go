...
func TestRollbackWithSpill(t *testing.T) {
	dir := t.TempDir()
	m := NewManager()
	m.SpillDir = dir
	m.SpillThreshold = 1
...
func TestIsCommitted(t *testing.T) {
	m := NewManager()
	// xid 0 should be less than counter=1 (after first Begin)
	tx := m.Begin()
	xid := tx.ID
...
func TestEnsureCounterAtLeast(t *testing.T) {
	m := NewManager()
...
func TestRollbackToSavepointSpillEmpty(t *testing.T) {
	dir := t.TempDir()
	m := NewManager()
	m.SpillDir = dir
	m.SpillThreshold = 1
...
func TestRollbackToSavepointSpillRewrite(t *testing.T) {
	dir := t.TempDir()
	m := NewManager()
	m.SpillDir = dir
	m.SpillThreshold = 1
...
func TestCommitApplyFnError(t *testing.T) {
	m := NewManager()
	tx := m.Begin()
	m.AddOp(tx, PendingOp{DB: "db", Table: "t"})
...
func TestCommitReadOpsSpillError(t *testing.T) {
	m := NewManager()
	tx := m.Begin()
	tx.spilled = true
...
func TestParallelLockTables(t *testing.T) {
	m := NewManager()
...
func TestEncodeOpCustomCodec(t *testing.T) {
...
func TestMultipleTableAccess(t *testing.T) {
	m := NewManager()
	tx := m.Begin()
...
func TestSavepointWithSpillNoOpCounter(t *testing.T) {
	m := NewManager()
	tx := m.Begin()
...
func TestIsConflictEdgeCases(t *testing.T) {
	tests := []struct {
		err  error
		want bool
	}{
...
func TestEnsureCounterAtLeastConcurrency(t *testing.T) {
	m := NewManager()
	var wg sync.WaitGroup
	for i := 0; i < 100; i++ {
		wg.Add(1)
...
func TestCommitWithRetryApplyFnErrorsOnConflict(t *testing.T) {
	m := NewManager()
